...
long long query(int l,int r,int id,int ql,int qr){
	if(l > qr || r < ql) return -1;
		...
		if(ql == l && qr >= r) yee += st[id].lmax;
		else if(ql <= l && qr == r) yee += st[id].rmax;
		else yee += st[id].sum;
		return st[id].midmax;
	} 
	else {
		...
		int lson = st[id].lson;
		int rson = st[id].rson;
		/*if(ql == l && qr >= m) yee += st[id].lmax;
		else if(ql <= l && qr == m) yee += st[id].rmax;
		else if(ql <= l && qr >= m) yee += st[id].sum;
		if(ql == l && qr >= m) yee += st[id].lmax;
		else if(ql <= l && qr == m) yee += st[id].rmax;
		else if(ql <= l && qr >= m) yee += st[id].sum;*/
		return max(query(l,m,lson,ql,qr),query(m + 1,r,rson,ql,qr));
	}
}
int main(){
	cin.tie(0);
	...
	while(q--){
		int l,r;
		cin >> l >> r;
		long long ans = max(query(1,n,0,l,r),yee);
		if(ans > 0) cout << ans << "\n";
		else cout << 0 << "\n";
		yee = 0;
	}
